...
결국, 어느날 대시보드에서 IoT 장비 여러대에 동시에 이상 발생이 표시되기 시작했다.

로그를 살펴보니, IoT 모니터가 주기보고를 수신하자마자 1초 이내에 처리하고 반환되어야 할 메시지 핸들러가 수 초 이상 동안 실행되고 있었다.

IoT 모니터의 메시지 핸들러가 처리하는 이상상태 감지는, 위에서 언급한 미리 저장해둔 직전 주기보고 데이터를 가져와서 현재 주기보고 데이터의 수신 시간과 비교해서 처리한다.
이 비교 결과, 장비의 보고 주기(예: 10초)보다 늦게 주기보고를 전달받은 경우, 장비나 네트워크에 이상이 발생하여 주기보고를 발행하지 못했다고 판단하게 된다.

IoT 모니터가 동일한 장비의 다음 주기보고를 수신했을 때 비교를 위해 조회한 직전 수신 데이터는 수 초에서 수십초 이전에 수신한 데이터였으며,
IoT 모니터는 그 시간 동안은 IoT 장비가 주기보고를 전달하지 않은 이상상태인 것으로 인지한다.
하지만 위에서 언급했던 것처럼, 메시지 핸들러에 추가적으로 구현된 처리들을 진행하면서 메시지 핸들러의 처리에 또다시 시간이 걸린다.
...
직전 주기보고 데이터를 조회하면, 메시지 핸들러의 처리가 지연되는 만큼 어느순간부터는 수십초 이전의 데이터가 조회되기 시작하므로
IoT 모니터는 이 수십초 동안 주기보고가 전달되지 않은걸로 인지하고 장비 이상 알림을 표시함.
...
이렇게 될 경우, 주기보고 데이터의 순서가 올바르다고 가정하고 개발된 기능들은 모두 무용지물이 되어, 해당 기능을 폐기하거나 별도로 새로 개발해야 할 수도 있을 것이다.

(나중에 듣기론, 그런 기능들은 진짜로 다 제거했다고 들었음-_-)
...
그래서 여러가지로 조사해본 결과, 출력채널의 종류를 변경해야 한다는 결론에 다다르게 되었다.
...
실제로 이렇게 개발을 다 해 놓았으나, 내부에서 이야기해본 결과 이 로직은 일단 사용하지 않는걸로 이야기 되었다.
...
위 문단에서 언급한, ```ApplicationContext```가 빈 목록을 재구성하지 못한다는 내용도 이거 하려다가 발견했던 이슈였다.

그런데 실제 코드에서는 메시지 채널을 생성하고 빈으로 등록한 다음, 그 빈을 다시 가져다 쓰지 않는다. 다시 가져다 쓰지도 않는데 굳이
...
단위 수를 늘려서 2000대라면? 8000대라면? 8000개의 스레드를 과연 서버가 버틸 수 있을까? ~~근데 사실 이쯤되면 모니터를 분리해야 하는거 아니야?~~
