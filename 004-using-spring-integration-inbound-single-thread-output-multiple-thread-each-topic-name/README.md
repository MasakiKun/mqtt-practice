...
이 프로젝트 코드는 와일드 카드가 사용된 MQTT 토픽에서 수신하고 있는 Spring Integration + MQTT 프로젝트에서,
각 토픽명 별로 메시지 핸들러가 별도의 스레드를 갖도록 구성된 예제 코드이다.

> 주의: 이 코드는 프로덕션 환경에 적용된 적이 없는 코드입니다. 코드를 참고하실때 감안하시기 바랍니다.
...
수신 데이터를 파싱하여 장비의 상태를 파악하며, 장비 이상 감지시 대시보드에 알람을 표시하는 구성으로 되어 있다.
...
결국 2에 의해서 메시지 핸들러는 점점 처리 시간이 길어지는데, 1에 의해서 데이터의 양이 감당할 수 없을만큼 늘어나서,
...
이 중 이상상태 감지하는 로직은, 위에서 언급한 미리 저장해둔 직전 주기보고 데이터를 가져와서 현재 주기보고 데이터의 수신 시간과 비교하는 부분이 있다.
이 비교 결과, 장비의 보고 주기(예: 10초)보다 늦게 주기보고를 전달받은 경우, 장비나 네트워크에 이상이 발생하여 주기보고를 못받았다고 판단하고 표시하게 된다.

어느날, 대시보드에서 IoT 장비 여러대에 동시에 이상 발생이 표시되어 살펴보니, 메시지 핸들러의 처리시간이 길어짐에 따라 MQTT로 전달된 메시지를 핸들러가 제때 처리하지 못하고 있었다.
...
그러나 이런 방법으로는 주기보고 데이터가 MQTT로 송신되었을 때, 이 데이터들의 순서를 보장하지 못하게 된다. 최악의 경우, 먼저 수신된 주기보고 데이터가
...
(나중애 듣기론, 주기보고 데이터의 수신 순서대로 판단하는 로직은 진짜로 다 제거했다고 들었음)
...
IoT 모니터링 시스템은 ```iotmon/#```을 구독하여 수신하게 되어 있었다.
...
기존에는 아예 주기보고 수신시마다 메시지 핸들러 스레드를 띄웠다고 했으니, 타협점으로 메시지 수신시마다 스레드를 띄우지 말고
토픽명 별로 핸들러 스레드를 할당해주면 MQTT에 데이터가 쌓이는 문제도 어느정도 해소도고
...
물론 Spring Integration에는 DirectChannel같은 구독형 채널(후술)의 성질을 가지면서도, 메시지 수신시 이후 처리를 별도의 스레드(정확히는 Executor)에게 위임하는, 
...
| 메시지 수신시 처리   | 채널에게 가입한 메시지 핸들러의 메서드를 직접 호출해서 전송 | 채널은 메시지를 저장하고, 외부에서 데이터를 가져갈 때까지 저장을 유지 |
...
위와 같이, 메시지 수신시에는 메시지를 저장만 할 뿐 아무것도 안하고 있다가(응?) 외부에서 receive() 메서드를 호출해서
데이터를 가져갈 때 메시지를 반환만 해주는 특성이 있다.

이런 특성으로 채널과 핸들러를 별로 클래스로 분리하기에 좋은 것 같으므로, DirectChannel을 QueueChannel로 변경한다.
